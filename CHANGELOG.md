...
- Refuse Claude backup inputs containing literal absolute home-path leaks, align documentation and CLI help with the current copied and merge-deployed Claude surface plus the separate `--skills` path, and require `bun test` to pass before the npm publish step can run.
...
- Merge-deploy bundled `agents/` and `workflows/` entries during `--claude` while leaving live paths absent from the bundle untouched. Differing real files and replaced symlinks are preserved under `${XDG_STATE_HOME:-$HOME/.local/state}/haoshoku/backups/<UTC-timestamp>/`, identical files are left unchanged, type conflicts are refused, and executable bundle modes are preserved; `mergeAgents()` now warns and leaves a same-named real local agent in place, while `--claude-backup` retains its symlink-skipping behavior.
- Refresh the bundled `agents/run-codex-task.sh` and `agents/test-launcher-lock.sh` from their live counterparts. The launcher gained an opt-in `--worktree-on-contention` mode and the lock suite gained the cases that prove it, and the bundle had neither. That refresh was made while `agents/` was backup-only; the merge-deploy entry above supersedes that interim policy, so the refreshed files are now bundle-owned deploy inputs. The two other drifted bundle files, `agents/codex-wrapper.md` and `agents/tldraw-offline.md`, are deliberately left alone: every difference in them is a private path that the bundle replaces with a placeholder, and this directory ships to public npm.
