...
- Prevent `haoshoku --claude` from deleting files and symlinks owned by other tools in `~/.claude/agents/` and `~/.claude/workflows/`. Claude directory deployment now makes ownership explicit: the exclusively Haoshoku-owned `conventions/`, `output-styles/`, and `hooks/` directories retain wipe-replace semantics, while the jointly owned `agents/` and `workflows/` directories merge bundled files into the live tree. Bundled files still deploy and overwrite same-named live files, but live-only entries—including symlinks merged into `agents/` by `skill_manager`—survive. A file removed from a future bundle may consequently linger in these merge directories; this is deliberate because the public npm tarball must not destroy data it did not create.
...
- Remove the retired routing-gate Stop hook from `configs/claude/hooks/`, including its script and 20-file, roughly 4,593-line test surface. The hook had already been deregistered from `settings.json` and moved under `~/.claude/.retired-20260726T181928Z/` on the live machine, but its old files remained in the tracked bundle. That mismatch was a resurrection path: `configure_claude.js` treats every `MANAGED_DIRS` entry as wipe-and-replace during `syncClaudeConfig()`, so applying the bundle would have deleted the already-clean live `hooks/` directory and copied the retired hook back into service. `MANAGED_DIRS` still contains `"hooks"`—the regression assertion in `tests/configure_claude.test.js` continues to guard the directory itself—while only the stale retired-hook contents were removed.
- Add `agents/` and `workflows/` to `MANAGED_DIRS`, making Haoshoku the single backup and deployment home for the Claude policy surface instead of leaving the Codex launcher, wrapper definition, validator hook, seat definitions, review station, and PR-review workflow in a separate private policy backup. `configs/claude/workflows/pr-review.js` also stops supplying installation-specific defaults for `repo`, `dir`, and `reviewFile`; all three arguments are now required and throw when omitted, because hidden environment-specific paths are a correctness defect in a general-purpose workflow regardless of repository visibility. Ten symlinked agents sourced from the separate skills repository were deliberately excluded from this first import because Git would have recorded only their link-target text, not their content; the following symlink-backup entry closes the open path that could otherwise have reintroduced them on a later backup.
- Prevent `backupClaudeConfig()` from capturing symlinked entries from any depth of a `MANAGED_DIRS` tree. `copyDirRecursive()` now accepts an opt-in `skipSymlinks` mode plus an `onSkipSymlink` callback, and only the `--claude-backup` call site enables it; skipped destinations are removed and `log.warning` names each source path, while `syncClaudeConfig()` and every other caller keep the existing symlink-preserving default. This matters because Git stores a symlink as its target text rather than the pointed-to file's bytes: a link into another repository can therefore look like a tracked backup while containing no usable content and resolving to nothing on another checkout. `tests/configure_claude.test.js` exercises real top-level and nested files alongside links to an external scratch file, proving the real content is preserved, both links are absent from the bundle, and both skipped paths are reported.
