...
- Refresh the bundled `agents/run-codex-task.sh` and `agents/test-launcher-lock.sh` from their live counterparts. The launcher gained an opt-in `--worktree-on-contention` mode and the lock suite gained the cases that prove it, and the bundle had neither — so a `haoshoku --claude` apply would have reverted the launcher, which is executed directly by every dispatch. The two other drifted bundle files, `agents/codex-wrapper.md` and `agents/tldraw-offline.md`, are deliberately left alone: every difference in them is a private path that the bundle replaces with a placeholder, and this directory ships to public npm.
