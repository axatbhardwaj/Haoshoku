...
- Refresh the bundled `agents/run-codex-task.sh` and `agents/test-launcher-lock.sh` from their live counterparts. The launcher gained an opt-in `--worktree-on-contention` mode and the lock suite gained the cases that prove it, and the bundle had neither — `agents/` is backup-only (`BACKUP_ONLY_DIRS`, see the 5.15.0 entry), so the config deploy never reads it and nothing was reverted; the refresh keeps the bundled snapshot honest, and it matters in the other direction, because `--claude-backup` copies live `agents/` into the bundle. The two other drifted bundle files, `agents/codex-wrapper.md` and `agents/tldraw-offline.md`, are deliberately left alone: every difference in them is a private path that the bundle replaces with a placeholder, and this directory ships to public npm.
