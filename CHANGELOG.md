...
- Give `safeCopyFile` separate backup roles: a write-once `.orig` first-capture slot and the existing rolling `.bak` previous-version slot. A fresh install with a pre-existing destination retains that user content in `.orig`; an upgraded install whose old releases already left `.bak` captures whatever is live on its next differing deploy even though `.bak` exists. This cannot recover an original that older releases already destroyed, and later hand-edits held only in rolling `.bak` are replaced by another differing deploy.
