...
function resolvePaths({
	...
	return {
		...
		pluginsDir:
			pluginsDir ?? path.join(home, ".config", "omarchy", "plugins"),
	};
}

function thirdPartyIds(bar) {
	if (!bar?.layout || typeof bar.layout !== "object" || Array.isArray(bar.layout)) {
	...
	for (const entries of Object.values(bar.layout)) {
		if (!Array.isArray(entries)) {
			throw new Error("Every configs/omarchy/bar.json layout section must be an array");
		}
		for (const entry of entries) {
			if (typeof entry?.id !== "string") {
				throw new Error("Every configs/omarchy/bar.json layout entry must have an id");
			}
...
function manifestUnknownIds(fsImpl, manifestPath, bar) {
...
export async function configureOmarchyBar({
	...
	if (!gate.ok) return { status: "refused", message: gate.message, changed: false };

	const { repoBarPath, manifestPath, liveShellPath, pluginsDir } =
	...
	return { status: "configured", changed };
}
...
export async function backupOmarchyBar({
	...
	const { repoBarPath, manifestPath, liveShellPath } = resolvePaths(pathOptions);
	if (!fsImpl.existsSync(liveShellPath)) {
		logImpl.warning(`No live Omarchy shell.json found at ${liveShellPath} — skipping`);
	...
	try {
		const { unknownIds } = manifestUnknownIds(fsImpl, manifestPath, live.bar);
		if (unknownIds.length > 0) {
			logImpl.warning(
	...
	fsImpl.writeFileSync(repoBarPath, `${JSON.stringify(live.bar, null, "\t")}\n`);
