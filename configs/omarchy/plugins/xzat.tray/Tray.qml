...
BarWidget {
  ...
  readonly property var embeddedWidgets: {
    var entries = TrayModel.normalizeEmbeddedWidgets(settings.embeddedWidgets)
    var registry = root.bar && root.bar.barWidgetRegistry ? root.bar.barWidgetRegistry.widgets : ({})
    return entries.filter(function(entry) {
      return registry[Util.canonicalWidgetId(entry.id)] !== undefined
    })
  }
  readonly property var pinnedItems: bucket("pinned")
  readonly property var drawerItems: bucket("drawer")
  readonly property var allItems: bucket("all")
  readonly property int drawerCount: TrayModel.drawerItemCount(drawerItems, embeddedWidgets)
  ...
  component EmbeddedWidget: Item {
    id: embeddedRoot

    required property var modelData
    readonly property string embeddedId: Util.canonicalWidgetId(String(modelData.id || ""))
    readonly property var registryComponent: {
      var registry = root.bar && root.bar.barWidgetRegistry ? root.bar.barWidgetRegistry.widgets : ({})
      return registry[embeddedId] ? registry[embeddedId].component : null
    }
    readonly property var activeItem: widgetLoader.item
    property var registeredBar: null
    property string moduleName: embeddedId
    property string region: "right"

    visible: activeItem ? activeItem.visible !== false : false
    implicitWidth: visible ? root.trayItemExtent : 0
    implicitHeight: visible ? root.trayItemExtent : 0
    ...
    function injectProps() {
      ...
      if ("moduleName" in target) target.moduleName = embeddedId
      if ("settings" in target) target.settings = modelData
    }

    function syncBarRegistration() {
      if (registeredBar === root.bar) return
      if (registeredBar && typeof registeredBar.unregisterModuleSlot === "function")
        registeredBar.unregisterModuleSlot(embeddedRoot)
      registeredBar = root.bar
      if (registeredBar && typeof registeredBar.registerModuleSlot === "function")
        registeredBar.registerModuleSlot(embeddedRoot)
    }

    Loader {
      id: widgetLoader
      anchors.fill: parent
      sourceComponent: embeddedRoot.registryComponent
      onLoaded: {
        embeddedRoot.injectProps()
        Qt.callLater(embeddedRoot.injectProps)
      }
    }

    Connections {
      target: root
      function onBarChanged() { embeddedRoot.syncBarRegistration() }
    }

    Component.onCompleted: {
      syncBarRegistration()
      Qt.callLater(syncBarRegistration)
    }
    ...
    onModelDataChanged: injectProps()
  }
}
