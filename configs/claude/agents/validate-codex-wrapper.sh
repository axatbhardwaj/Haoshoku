#!/bin/bash
# PreToolUse hook for the named Codex wrapper agents. FAILS CLOSED.
...
safe_wrapper_route() {
  ...
  for ((i = 1; i < ${#words[@]}; i++)); do
    ...
    case "$token" in
      --model)
        i=$((i + 1)); model=${words[$i]:-}; model_count=$((model_count + 1))
        ;;
      --mode)
        i=$((i + 1)); mode=${words[$i]:-}; mode_count=$((mode_count + 1))
        ;;
      --attribution-path)
        i=$((i + 1)); attribution_path=${words[$i]:-}; attribution_count=$((attribution_count + 1))
        ;;
      --wait)
        return 0
        ;;
