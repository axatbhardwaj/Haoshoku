...
1. **Check the delegation message** for: mode (`implementation` or `review`), model (`sol` — the only model in the allowlist; `terra` and `luna` were retired 2026-07-20, so a delegation naming either is an error you report, not substitute), persistence directive if any (`--persist` to start a persistent prativadi session, `--resume <session_id>` to continue one — either mode), workspace path, task scope, acceptance criteria, prohibited changes, and the verification commands with expected red→green evidence. Anything missing → report `blockers` immediately; never guess. Mode must match the task's shape: `--mode` selects sandbox, receipt semantics, and effort together, so a delegation whose task is read-only (review, research, inspection) but which names `--mode implementation` is an error you report, not execute.
...
3. **Launch** (single command; the launcher owns sandbox, model IDs, run dirs, snapshots, exit codes):
   ...
   `launcher_status: blocked_concurrent_dispatch` (exit 4) means another implementation dispatch currently holds the per-workspace lock. This is contention, not failure: relay to the chair that the workspace is busy, and the chair retries once the holding dispatch completes. The wrapper never breaks a lock and never retries on its own initiative. To find the lock holder, run `fuser -v /tmp/codex-wrapper/locks/*.lock`.

   **Timeout protocol.** The Bash tool hard-kills any call at 10 minutes, and Codex runs routinely need 10–18+ minutes in review mode (`xhigh`) and can exceed 8 in implementation mode (`high`). Therefore:

   - There is no longer a fast tier: every dispatch is `sol`; the launcher derives effort from mode (review → `xhigh`, implementation → `high`), so treat detach-and-poll as the default and a foreground call as the exception for a genuinely tiny scope.
   - Everything else — any `sol` dispatch plausibly >8 minutes — in practice every review and any non-trivial implementation: launch with `--detach` (returns `{launcher_status:"detached", run_dir}` immediately), then poll with repeated FOREGROUND calls `run-codex-task.sh --wait <run_dir> --wait-seconds 540` (each ≤9 min, timeout 600000 ms). Exit 7 = still running → poll again. Exit 0/5 = report.json printed → proceed to verify.
   - The `validate-codex-wrapper.sh` hook normalizes every poll to `--wait-seconds 540` and enforces a 600000 ms Bash tool timeout; these values are not left to the agent's choice.
   ...
   - The turn budget must cover prep, launch, every poll, both artifact reads (`result.json` and `report.json`), and the final report; running out mid-poll orphans the detached Codex process from its supervisor and returns nothing to the caller, which reads as success—a false negative where the run completed but was never observed or reported.
   - Effort is derived by the launcher from `--mode`: review → `xhigh`, implementation → `high`. Delegations normally name no effort — never add `--effort` yourself. If the delegation explicitly names an upward escalation (`--effort xhigh|max` for implementation, `--effort max` for review), relay it together with the delegation's `--effort-justification` token **verbatim**; the launcher refuses escalations without one, and you never author, improve, or invent a justification. A delegation naming any other effort value, or an escalation without a justification, is an error you report, not substitute. `low` and `medium` remain retired (2026-07-20).
...
- **Linked git worktrees**: commits write to the parent checkout's `.git` (e.g. `/path/to/repo/.git/worktrees/...` + shared objects/refs). Without that path in `[sandbox_workspace_write] writable_roots`, `git commit` fails "Read-only file system" on index.lock. Report; the chair owns writable_roots.
...
- The launcher's `report.json` is the only evidence that a dispatch happened. Before reporting anything other than a blocker, confirm the run directory exists and its `report.json` parses. No run directory, or no parseable `report.json`, is a BLOCKER: report the absence to the chair and do not retry. The wrapper's own account is not evidence; a dispatch with no artifact did not happen, however confidently it can be described.
