...
| Five wrapper agents, two role definitions, two native planner/reviewer agents, launchers (`run-codex-task.sh` and `run-opencode-seat.sh`), validation hook, result schema, and render-workspace preparer | `~/.claude/agents/` | Deploy |
