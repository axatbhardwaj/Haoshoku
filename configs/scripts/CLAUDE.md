...
| `claude-desktop-toggle` | Super+I handler for the native `claude-desktop` app (pacman `claude-desktop`, class `claude-desktop`) alongside the ChatGPT Brave PWA. Reclaims or launches both windows, moves them onto `special:claude-desktop`, focuses DP-2, and floats them as a top/bottom stack (Claude on top) that respects Hyprland's reserved left Caelestia bar. Cannot be named `claude-desktop` itself: `~/.local/bin/` precedes `/usr/bin/`, so that basename would shadow the real app binary. Replaced the old `ai-webapps-toggle` (which paired Claude's *Brave PWA* with ChatGPT — Claude is native now). | Changing the claude-desktop / ChatGPT launch, placement, or Super+I behavior |
