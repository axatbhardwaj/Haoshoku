...
| `crunchyroll-hd` | Native Brave app-mode launcher for Crunchyroll in a dedicated `~/.local/share/crunchyroll-brave-profile`. No Wine bottle, no CDP, no seeding (log in once) — Crunchyroll plays 1080p natively in Linux Brave. Routed to `special:crunchyroll` by app-id class `brave-www.crunchyroll.com__-Default`; `Super+Shift+C` (which displaces hyprpicker to `Super+Alt+C`). | Changing the Crunchyroll URL, profile, or launch flags |
| `jiohotstar-hd` | Native Brave app-mode launcher for JioHotstar in a dedicated `~/.local/share/jiohotstar-brave-profile`, same pattern as `crunchyroll-hd` (no seeding; log in once). Routed to `special:jiohotstar` by app-id class `brave-www.jiohotstar.com__-Default`; `Super+Shift+J`. Plays via the software video path (`brave-flags.conf`); resolution is Linux/Widevine-served, not guaranteed 1080p. | Changing the JioHotstar URL, profile, or launch flags |
