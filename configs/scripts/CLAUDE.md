...
| `crunchyroll-hd` | Native Brave app-mode launcher for Crunchyroll (personal Default profile). No Wine bottle, no CDP, no profile seeding — Crunchyroll plays 1080p natively in Linux Brave. Routed to `special:crunchyroll` by app-id class `brave-www.crunchyroll.com__-Default`; `Super+Shift+C` (which displaces hyprpicker to `Super+Alt+C`). | Changing the Crunchyroll URL, profile, or launch flags |
| `jiohotstar-hd` | Native Brave app-mode launcher for JioHotstar (personal Default profile), same pattern as `crunchyroll-hd`. Routed to `special:jiohotstar` by app-id class `brave-www.jiohotstar.com__-Default`; `Super+Shift+J`. Plays via the software video path (`brave-flags.conf`); resolution is Linux/Widevine-served, not guaranteed 1080p. | Changing the JioHotstar URL, profile, or launch flags |
