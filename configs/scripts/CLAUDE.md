...
| `haoshoku-zed-glass` | Post-processes Omazed's generated Zed theme with the verified dotted `background.appearance` transparency field, alpha-adjusted base surfaces, and neutral border reassignment. | Changing Zed transparency, Omazed theme hooks, or the protected opaque/accent surfaces |
| `haoshoku-chromium-flux` | Launches Chromium on the isolated flux profile. Paired with `configs/mimeapps/applications/chromium.desktop`; its basename must remain a single whitespace-free `Exec` first field because `omarchy-launch-webapp` extracts only that field. | Changing the Flux Chromium profile, wrapper name, or paired desktop entry |
